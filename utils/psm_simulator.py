...
class PSMSimulator:
    ...
    def __init__(
        ...
        # assert(self.stability < np.sqrt(2) / np.pi)        # ç¨³å®šæ€§æ¡ä»¶

        # self.tflimit = self.dt * self.vmax / self.dmax     # æ—¶é¢‘é‡‡æ ·å®šç†
        # print("tflimit:", self.tflimit)
        # assert(self.tflimit < 1)

        # self.splimit = self.dmax * 2 * self.fm / self.vmin # ç©ºé—´é‡‡æ ·å®šç†
    ...
    def cal_psm_dz(self, u):
         return np.real(np.fft.ifft2(1j * self.kx * np.fft.fft2(u)))

    def cal_psm_dx(self, u):
         return np.real(np.fft.ifft2(1j * self.kz * np.fft.fft2(u.T)).T)

    def cal_psm_ddz(self, u):
        return np.real(np.fft.ifft2(-self.kx**2 * np.fft.fft2(u)))
    
    def cal_psm_ddx(self, u):
        return np.real(np.fft.ifft2(-self.kz**2 * np.fft.fft2(u.T)).T)
    ...
    def forward(self):
        ...
        # # # d^2 U = 
        self.ux, self.ux1 = 2 * self.ux - self.ux1 + self.rho * (\
        ...
        self.uz, self.uz1 = 2 * self.uz - self.uz1 + self.rho * (\
...
## Old version
## Date: Apr 14 22:28
## This version only can calculate å„é¡¹åŒæ€§
